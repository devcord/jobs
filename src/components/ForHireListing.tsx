...
export const ForHireListing: FC = () => {
  ...
  const handleJobPost = async () => {
    ...
    addDoc(collection(database, "hire"), jobData).then((docRef) => {
      const docId = docRef.id;
      setStatus('SUCCESS');
      setMessage(`For Hire Listing posted successfully with ID: ${docId}. Please wait for approval.`);
    }).catch((error) => {
      setStatus('ERROR');
  ...
  return (
    <>
      {/* Create a form for submitting a for hire listing */}
      <div className="flex flex-col mt-8">
        <h2 className="text-xl font-bold">Submit a for hire listing</h2>
        {/* Job Description */}
        ...
        <LoadingButton
          ...
          disabled={!jobDescription || !tags.length || session === null}
          style={{
            marginTop: '1rem'
          }}
